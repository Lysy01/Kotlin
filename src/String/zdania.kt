...
fun main() {
    ...
    var imie: String? = "Maciek"
    if (imie != null) {
        println("Ja mam jedno ciastko, $imie ma drugie")
    } else {
        println("Ja mam jedno ciastko")
    }
    ...
    for (i in wierszCharArray.indices) {
        if (wierszCharArray[i] == '0') { // ' ' dlatego bo sprawdzamy pojedynczy znak
            wierszCharArray[i] = 'o'
        }
    ...
    val tekstOcenzurowany = tekstcenzura.replace("kurcze", "******")
